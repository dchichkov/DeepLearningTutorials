...
def test_dA( learning_rate = 0.1, training_epochs = 15, dataset ='../data/mnist.pkl.gz',
        batch_size = 200, output_folder = 'dA_plots' ):
    ...
    #X = numpy.array(train_set_x.value[0:batch_size])
    #for x in X:
    #    d = 28*28//5
    #    j = numpy.random.randint(28*28 - d)
    #    x[j:j+d] = X[numpy.random.randint(len(X))][j:j+d]

    
    corrupt_input = theano.function([],
                               [da.get_corrupted_input(input = x, corruption_level = 0.3)], 
                               givens = {x:train_set_x[0:batch_size]})
    X = corrupt_input()[0]
    image = PIL.Image.fromarray(tile_raster_images( X = X,
                 img_shape = (28,28),tile_shape = (10,10), 
                 tile_spacing=(1,1), scale_rows_to_unit_interval = False))
    image.save('input_corruption_30.png') 
